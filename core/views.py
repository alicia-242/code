...
def valider_preinscription(request, preinscription_id):
    ...
    try:
        send_mail(
            ...
            f'Bonjour {preins.nom}, votre inscription a été validée. Identifiant : {user.username}.',
            'ttg@example.com',
            [preins.email],
            fail_silently=False
        )
...
@login_required
def moyenne_view(request, matricule):
    etudiant = Etudiant.objects.get(matricule=matricule)
    notes = Note.objects.filter(etudiant=etudiant)
    total = sum(note.valeur * note.coefficient for note in notes)
    somme_coefficients = sum(note.coefficient for note in notes)
    moyenne = total / somme_coefficients if somme_coefficients else 0
    return render(request, 'moyenne.html', {'etudiant': etudiant, 'moyenne': moyenne})
...
@login_required
def modifier_emargement_view(request, emargement_id):
    if not request.user.is_staff:
        return HttpResponseForbidden("vous n'avez pas la permission de valider une inscription")
    emargement = get_object_or_404(Emargement, id=emargement_id, formateur=request.user)
    if request.method == 'POST':
        form = EmargementForm(request.POST, instance=emargement)
        if form.is_valid():
            form.save()
            return redirect('liste_emargements')
    else:
        form = EmargementForm(instance=emargement)
    return render(request, 'modifier_emargement.html', {'form': form})

@login_required
def supprimer_emargement_view(request, emargement_id):
    if not request.user.is_staff:
        return HttpResponseForbidden("vous n'avez pas la permission de valider une inscription")
    emargement = get_object_or_404(Emargement, id=emargement_id, formateur=request.user)
    if request.method == 'POST':
        emargement.delete()
        return redirect('liste_emargements')
    return render(request, 'supprimer_emargement.html', {'emargement': emargement})
